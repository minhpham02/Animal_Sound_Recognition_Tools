...
def funcFrequencyMagnitude(audio_dir):
    # audio_dir = ""
    audio, sr = librosa.load(audio_dir, duration = 3)#Load file âm thanh vào librosa
    ...
    for i in range(0, len(X_mag)):        
        if(X_mag[i] > max):
            ...
            pos = i
        if(i> 0):
            if( (i % int(len(X_mag)/3)) == 0 and i != len(X_mag)-1):            
                freq[position] = pos
